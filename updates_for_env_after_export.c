...
/*   Updated: 2022/12/20 13:29:34 by hchahid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	replace_value(t_env **env, char *compared_to, char *env_var)
{
	t_env	*iter;
...
int	does_env_exist(t_env **env_p, char *env_var)
{
	...
	while (iter)
	{
		if (replace_value(&iter, compared_to, env_var))
			return (free(compared_to), 1);
		iter = iter->next;
	}
	free(compared_to);
	return (0);
...
void	change_env_value(t_env *env, char *value)
{
	...
	i = 0;
	free(env->e_value);
	env->e_value = get_e_var_value(value);
}
